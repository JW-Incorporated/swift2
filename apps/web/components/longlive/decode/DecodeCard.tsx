    ...
    >
      ...
      <div className="px-5 pb-5 pt-4">
        {revealed ? (
          ...
          <LockedPayoff eraLabel={payoffEra.shortName} eraColor={payoffColor} eraFg={accentFgFor(payoffEra.theme)} teaseLine={Boolean(clue.payoff.line)} />
        )}
...
function LockedPayoff({ eraLabel, eraColor, eraFg, teaseLine }: { eraLabel: string; eraColor: string; eraFg: string; teaseLine: boolean }) {
  return (
    <div aria-hidden className="w-full select-none rounded-xl border border-dashed p-4 text-left" style={{ borderColor: 'var(--era-line)', background: 'var(--era-surface-2)' }}>
      <div className="mb-3 flex items-center gap-2">
        ...
        {/* #3663: raw accent as text on its own tint fails WCAG 1.4.3 on several
            eras; the chip is colored by the payoff's era, not the page's, so the
            fix is #3318's shape — solid accent fill + accentFgFor foreground. */}
