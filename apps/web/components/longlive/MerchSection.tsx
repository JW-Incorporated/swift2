...
/**
 * The Merch pane of the combined Community section (PLAN.md 2026-08-14 steps
 * 4-5 — mounted by `CommunitySection.tsx`, which owns the section's "Community"
 ...
 * no on-site payment or checkout — every card is a plain outbound link, same
 * seam as MomentDetail's "Shop the look" (buildShopUrl / isAffiliate /
 * SHOP_DISCLOSURE from shop.ts), so the affiliate flip stays the one-file
 * change that seam promises.
 *
 ...
 * showing into era sections (newest era first) and drives the pane's own
 ...
 * `shopTheLook` has data, so the row renders nothing and the pane just shows
 * that bucket's era sections directly — no dead toggle, no empty stub.
 *
 * Cross-navigation: a `source` back-reference opens that exact moment via
 * the existing `useAppActions().openItem` overlay — no new navigation
 * plumbing, the same action MomentDetail itself uses to open.
 */
