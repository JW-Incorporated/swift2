...
export function MerchStyleSection() {
  ...
  const filterMatched = useMemo(
    () => MERCH_CATALOGUE.shopTheLook.filter((item) => merchMatchesFilter(item, activeFilters)),
    [activeFilters],
  );
  ...
  const eraSpineEntries: readonly EraSpineEntry[] = useMemo(() => {
    ...
    return [
      { key: 'all', name: 'All eras', year: 'All', color: allGradient, count: filterMatched.length },
      ...ERAS.map((era) => ({
        key: era.id,
        name: era.shortName,
        year: era.yearLabel,
  ...
  const filteredItems = useMemo(() => {
    const pool = activeEraKey === 'all' ? filterMatched : filterMatched.filter((item) => item.source?.eraId === activeEraKey);
    const dateOf = (item: MerchItem) => (item.source ? (getContentItem(item.source.momentId)?.date ?? '') : '');
    return [...pool].sort((a, b) => dateOf(b).localeCompare(dateOf(a)));
  }, [filterMatched, activeEraKey]);

  useEffect(() => setVisibleCount(PAGE_SIZE), [activeEraKey, activeFilters]);
  ...
  const filtersActive = activeEraKey !== 'all' || activeFilters.size > 0;
  const tallyLabel = [
    activeEra ? `${activeEra.shortName} (${activeEra.yearLabel})` : null,
    activeFilters.size > 0 ? [...activeFilters].map((id) => MERCH_FILTER_LABEL[id]).join(', ') : null,
  ]
    .filter(Boolean)
    .join(' · ');
    ...
    <>
      <div>
        ...
        <div className="-mt-px flex flex-wrap items-center gap-3 border-x border-b border-[color:var(--merch-line-strong)] bg-[color:var(--merch-ink-2)]/60 px-[18px] py-[11px]">
          <span className="text-[10px] uppercase tracking-[0.22em] text-[color:var(--merch-muted)]">Narrow by</span>
          <div className="flex flex-wrap gap-px">
            <FilterPill active={activeFilters.size === 0} accent="var(--merch-lilac)" onClick={clearFilters}>
              All items
            </FilterPill>
            {ALL_MERCH_FILTERS.map((id) => (
              <FilterPill key={id} active={activeFilters.has(id)} accent="var(--merch-lilac)" onClick={() => toggleFilter(id)}>
      ...
      <div className="my-6 flex flex-wrap items-center gap-3 text-[11px] uppercase tracking-[0.16em] text-[color:var(--merch-muted)]">
        <span>
          Showing <b className="font-medium text-[color:var(--merch-cream)]">{pageItems.length}</b> of{' '}
          <b className="font-medium text-[color:var(--merch-cream)]">{filteredItems.length}</b> looks
        </span>
        {filtersActive && (
          <>
            <span>
