...
import { clamp, scrollTopForMonthFloat, type ScrubberModel } from './scrubberModel';
...
export interface GrabScrub {
  grabbed: boolean;
  /** Live month float while grabbing (for a variant to read on its own frames). */
 ...
 * month — the v1 era-only snap, honoured across all four variants.
 */
export function useGrabScrub(opts: GrabScrubOptions): GrabScrub {
  ...
  const scrub = useCallback(
    (e: ReactPointerEvent) => {
      ...
      const mf = fractionToMonthFloat
        ? fractionToMonthFloat(f, model)
        : f * model.totalMonths;
  ...
  const onPointerUp = useCallback(() => {
    setGrabbed(false);
    // Snap to the nearest era's first month (v1 era-only snap).
    const mf = grabMonthFloatRef.current;
    const metas = model.eraMetas;
