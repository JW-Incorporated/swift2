 ...
 * overlay needs a param that reopens it; #2105 added `mode` for the three
 ...
 * addresses a whole surface (the Threads gallery, Mood, or Clownbot), never
 * anything more specific than that. Exactly one param is ever present on a
 * shared URL, but the order keeps the helper total when a URL is hand-mangled.
 */
...
export type DeepLinkTarget =
  ...
  | { kind: 'lens'; id: string }
  | { kind: 'mode'; mode: 'threads' | 'mood' | 'clownbot' }
  | { kind: 'era'; id: string }
  | null;

const MODE_VALUES = new Set(['threads', 'mood', 'clownbot']);
...
export function deepLinkTarget(search: string, validLensIds: readonly string[]): DeepLinkTarget {
  ...
  // #2105 — Threads gallery, Mood, and Clownbot each carry no user input, so
  // the whole surface (not any one thing on it) is the shareable target.
  const mode = params.get('mode');
  if (mode && MODE_VALUES.has(mode)) {
    return { kind: 'mode', mode: mode as 'threads' | 'mood' | 'clownbot' };
  }
