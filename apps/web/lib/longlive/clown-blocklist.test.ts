 ...
 * combined gate in clown-safety.test.ts). Cases below are drawn from the
 * red-team pass build A's clownbot-safety.test.ts pinned for the topic
...
describe('SEXUALIZATION_TERMS mirror', () => {
  it('is mirrored from scripts/content-engine/config.mjs safety.sexualizationTerms', () => {
    ...
    // clown-blocklist.ts. This just pins that the mirror has not been
    // silently thinned.
