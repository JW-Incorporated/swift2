// Clownbot's no-DB fallback lore — the seed-authored source for
// apps/web/lib/longlive/clownbot-lore.generated.ts.
//
// Migrated 2026-09-04 per Fable ruling FR-t_2745eb60-1 (issue #3515): this
// content used to be hand-edited directly in apps/web/lib/longlive/
// clownbot-lore.ts by the unattended Rumor Desk lane, which put unattended
// content editing directly on runtime app source — a hard limit violation.
// The ruling moved the editing surface here, into the seed-only Vault Run
// lane, while keeping the generated artifact's shape identical.
//
// Lives in its own supabase/seed/clownbot-lore/ directory (not
// supabase/seed/content/) on purpose: scripts/sync-longlive-content.mjs
// scans every *.mjs under supabase/seed/content/ as vault-moment content
// with a completely different { eraSlug, items } shape — dropping this file
// in there would have it silently mis-parsed by that generator too.
//
// REFRESH PATH: docs/content-ops/clownbot-rumor-refresh.md and
// docs/agents/runner-prompts/vault-lanes/4-rumor-desk.md Part C. Edit THIS
// FILE, then run `npm run sync:content` (or
// `node scripts/sync-clownbot-lore.mjs` directly) to regenerate
// clownbot-lore.generated.ts. Never hand-edit the generated file.
//
// THE ONE RULE, unchanged: no source, no ship. Every item carries at least
// one named outlet with a real https URL and a real date — enforced both by
// scripts/sync-clownbot-lore.mjs (drops anything that doesn't qualify, same
// as the era-secrets / theories generators) and by
// apps/web/lib/longlive/clownbot-lore.test.ts on the built output.
//
// Schema (mirrors the `LoreItem` type in apps/web/lib/longlive/types.ts —
// see docs/content-ops/clownbot-rumor-refresh.md for the full field guide):
//   id, status ('rumor'|'reported'|'confirmed'|'debunked'), date,
//   lastCheckedOn, headline, detail, sources: [{ name, url }], prompts?,
//   ledger?: { theory, verdict: 'clowned'|'confirmed', on }, evergreen?, tags?
//
// Bump `updatedOn` below on every editorial sweep — it becomes
...
export default {
  ...
  items: [
    ...
    },
    {
      ...
      sources: [
        {
          name: 'Wikipedia — The Life of a Showgirl',
    ...
    },
    {
      ...
      sources: [
        ...
        {
          name: 'Wikipedia — Super Bowl LX halftime show',
