...
| `scripts/social/lib/inbox.mjs` | Wave 3 T1 (T6 slice): minimal `v:1` intent schema (`validateIntent`) + `readIntents(dir)` — selection/expiry/scoring/displacement stay Wave 4's job |
...
| `scripts/social/weekly-brief.mjs` | Wave 3 T4: `buildWeeklyBrief` — the Monday `#longlive-social` post (5-line scorecard, what changed, 14-day calendar, ≤3 numbered proposals, ≤2 questions); tests in `weekly-brief-workflows.test.ts`, thread ingestion in `social-approval-poll-plan-brief.test.ts` |
...
| `social/metrics/posts/<YYYY-MM>/<postId>.json` | Wave 4 T3: per-post Instagram engagement (`like_count`/`comments_count` off the IG media node, `postId` = `platformPostId` from `social/posted/`), written/refreshed daily by `growth-snapshot.mjs` for every Instagram post still inside its 30-day window. IG-only v1 — no `impressions`/`reach`/`saved`/`shares`, no X per-post reads (`HUMAN-ACTIONS.md` #62/#63) |
