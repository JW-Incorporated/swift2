...
| `docs/specs/marjorie-overhaul/m5-chat.md` | M5 spec: 5-min Discord poll claims founder messages with a 👀 reaction, dispatches a per-message Opus chat routine (Marjorie acts then answers, Tree read-mostly), a `run:` step posts the reply in-thread and reacts ✅/❌; bot token never in the agent env |
