...
| `scripts/marjorie/lib/clock-watch.mjs` | M7: the poll's clock watch, only while `CLOCK_LIVE` — the newest person-started `workflow_dispatch` of `bot-chat-poll.yml` in 24 h; 20 min or older with no open `Clock is not firing` alert → `bot-chat-alarm.yml` `clock-silent`; never fails the poll; also used by `chat-alarm.mjs check`; tests: `clock-watch.test.ts` |
