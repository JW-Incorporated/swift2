...
- `apps/web/lib/longlive/filter-chips.tsx` — the shared chip row, extracted so `FilterBar` and merch use ONE implementation (forking it would have been the 4th "two mechanisms for one fact"). **`.tsx` not `.ts` — it contains JSX.** Its two `#fff` literals predate the extraction; known exception to the no-raw-hex rule.
- `apps/web/components/longlive/SectionJumpBar.tsx` + `lib/longlive/section-jump.ts` — the jump-and-preview control, two states (in-flow grid, and one line inside the sticky rail). **Every content chip carries its count — the counts ARE the depth preview, so never render one without.** Outline styling ONLY: solid-fill means "filter" here, and both sit near the top of the Merch pane. `section-jump.ts` owns the DOM-id contract and `scrollTargetY`. **Scroll-spy must scroll the chip row's own `scrollLeft`, never `scrollIntoView`** — inside a sticky ancestor that drags the page and breaks an in-flight jump.
