...
| `scripts/marjorie/reply-poll.mjs` | M2 (moved from M4): plain-script reply poller — reads founder replies from the brief's Discord thread (`discord-message-id` marker comment locates it) and relays each as an issue comment, idempotent via `<!-- relay-id: ... -->`; never an agent step |
| `.github/workflows/marjorie-reply-poll.yml` | M2: cron + `workflow_dispatch` job invoking `reply-poll.mjs` under `environment: social` (reuses the existing `DISCORD_BOT_TOKEN`, no new environment) |
...
| `scripts/marjorie/lib/loop-asks.mjs` | M4/L1: pure ask parsers (`needsFromMarjorie` JSON, `- For Tree:` line), `loop-ask` marker + key, `findFiled` trust by `github-actions` login, `fileAsk`, incoming-ask selection, both brief renderers; tests: `loop-asks.test.ts` |
| `scripts/marjorie/loop-asks.mjs` | M4/L1: CLI `file-tree` (`routine-tree-weekly-plan.yml` send-brief → `--loop` block) and `file-marjorie` (`routine-marjorie-brief.yml` deliver → number written into the brief); soft-fail, exit 0 on GitHub errors |
