...
| `scripts/social/lib/lessons.mjs` | Wave 3 T5: `parseLessons`/`renderLessons`/`nextId` — pure round-trip parser/renderer for `social/lessons.md`; `validate-queue.mjs` is the one caller that reads the file from disk and threads `activeLessonIds` into `queue-schema.mjs`'s `validateQueueItem` |
