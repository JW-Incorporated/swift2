...
| `docs/plans/marjorie-overhaul/waves/*.md` | Paste-ready prompts, one per wave: `m0-design` (done), `m1-comms`, `m2-watchdog-handling`, `m3-triage`, `m4-loop`, `m5-chat` |
...
| `scripts/marjorie/chat-poll.mjs` | M5 I/O: `poll` claims founder messages in #longlive-marjorie / #longlive-tree and their active threads with 👀 (paged back to 24 h), then dispatches `routine-marjorie-chat.yml` / `routine-tree-chat.yml`; never removes a 👀 — reconciles 👀-without-✅/❌ against the routine's runs by `run-name` (no run → re-dispatch, finished run → ❌ then `[chat failed]`); unreadable places or blank bodies fail the run; `context` writes one message's context JSON for a chat routine; tests: `chat-poll.test.ts` |
| `scripts/marjorie/lib/chat-inbox.mjs` | M5 pure half of the poll: `BOTS` table, `runTitle` (the chat routines' `run-name`), `founderIds` (`DISCORD_FOUNDER_IDS` or `approvers.mjs`), `selectInbox` (picked ≤3 oldest / claimed / blank-body), `dispatchArgs`, `buildContext`; tests: `chat-poll.test.ts` |
| `.github/workflows/bot-chat-poll.yml` | M5: the one Discord poll job (`environment: social`, `actions: write`, `issues: write`) — `chat-poll.mjs poll`, then `reply-poll.mjs` — on the reply poller's old :03/18/33/48 slots (not every 5 min: the org's Actions budget is a $10/month hard stop); `BOT_CHAT_ENABLED=false` kill switch, `dry_run` input; watched by `watchdog.yml` |
| `.github/workflows/routine-marjorie-chat.yml` | M5: dispatch-only, one run per founder message (`run-name: Marjorie chat · <message id>`, the poll's reconcile key; concurrency per message). Jobs `context` (`social`: context JSON, reply thread, stops a duplicate ✅/❌ run) → `run` (template, Opus, 25 turns, no Write/Edit, PAT + `GH_DISPATCH_TOKEN`) → `post` (`ops`: reply or `[chat failed]` via Marjorie's webhook) → `finish` (`social`: ✅/❌, bot-token `[chat failed]` fallback, turn log, deletes the run's chat artifacts — public repo); `force_fail` input skips the agent |
...
| `scripts/marjorie/chat-post.mjs` | M5: `save` (agent writes `.scratch/out/chat-reply.md` + summary without a Write tool), `thread` (starts the reply thread; `skip=true` on a duplicate run), `post` (reply or `[chat failed]` via the bot's webhook; mentions neutralized and `ref:` lines defused before the 1800-char cap so it is always one message; with the bot token, a re-run on a ✅/❌ message posts nothing), `finish` (fresh read; ✅, or the referenced bot-token `[chat failed]` notice before ❌; metadata-only `💬 chat:` turn log with `<!-- chat-id -->` — public repo); tests: `chat-post.test.ts` |
...
| `.github/workflows/routine-tree-chat.yml` | M5: dispatch-only Tree chat (`run-name: Tree chat · <message id>`, concurrency per message). Jobs `context` (`social`) → `run` (template, Opus, 25 turns, `Bash(gh:*),Bash(node:*),Read,Grep,Glob` — no git, PAT or dispatch token) → `deliver` (`social`: reply or `[chat failed]` via the repo-secret Tree webhook, ✅/❌, turn log, deletes the chat artifacts); `force_fail` skips the agent |
...
| `docs/specs/marjorie-overhaul/m5-chat.md` | M5 spec: a 15-min Discord poll (budget-bound, see Mechanics 1) claims founder messages with a 👀 reaction, dispatches a per-message Opus chat routine (Marjorie acts then answers, Tree read-mostly), a `run:` step posts the reply in-thread and reacts ✅/❌; bot token never in the agent env |
