...
| `scripts/marjorie/chat-poll.mjs` | M5: `poll` claims founder messages in #longlive-marjorie / #longlive-tree and their active threads with 👀, then dispatches `routine-marjorie-chat.yml` / `routine-tree-chat.yml` (24 h window, ≤3 per channel, oldest first, founder ids from `DISCORD_FOUNDER_IDS` or `approvers.mjs`, skips a bot whose routine isn't on main); `context` writes one message's context JSON for a chat routine; tests: `chat-poll.test.ts` |
