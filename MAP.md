...
| `components/longlive/{EraThreadsPivot,TopBar}.tsx` | The "Threads running through {era}" strip below the feed; sticky top bar + `ModeToggle` (now six tabs), hosts `TimelineScrubber` in era mode. Labels vs icon-only is decided in `lib/longlive/bottom-nav-layout.ts`: `entries.length >= BOTTOM_NAV_ICON_ONLY_THRESHOLD` (5), so **five tabs is already icon-only — four is the last count that keeps text** |
...
- `data/{communities.json,communities-report.md}` + `sources.md` — 30 verified communities across 8 platforms, each with verification provenance; the landscape narrative and what is deliberately absent; every directory/thread/article mined plus the platform blockers, so the research is re-runnable. The JSON is the SOURCE — the app reads the transcribed copy below, so a refresh must update both.
...
- `apps/web/components/longlive/{Community,Merch}Section.tsx` — directory grouped by platform (verification badge shows only when NOT verified; flags render above descriptions); merch links out only, no cart, no checkout (item 4a standing rule).
