...
| `components/longlive/{EraThreadsPivot,TopBar}.tsx` | The "Threads running through {era}" strip below the feed; sticky top bar + `ModeToggle` (now six tabs, icon-only below `BOTTOM_NAV_ICON_ONLY_THRESHOLD`), hosts `TimelineScrubber` in era mode |
