...
| `scripts/marjorie/chat-post.mjs` | M5: `save` (agent writes `.scratch/out/chat-reply.md` + summary without a Write tool), `thread` (starts the reply thread; `skip=true` on a duplicate run), `post` (reply or `[chat failed]` via the bot's webhook, cut to 1800 chars with a run link), `finish` (✅/❌, bot-token `[chat failed]` when `post` died, `💬 chat:` turn log with `<!-- chat-id -->`); tests: `chat-post.test.ts` |
