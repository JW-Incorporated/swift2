...
| `docs/plans/marjorie-overhaul/waves/*.md` | Paste-ready prompts, one per wave: `m0-design` (done), `m1-comms`, `m2-watchdog-handling`, `m3-triage`, `m4-loop`, `m5-chat`, `m6-live-asks`, `m7-doorbell` |
...
| `scripts/marjorie/lib/discord.mjs` | C1: `post()` — chunked Discord webhook delivery, one retry (2s, or the 429 body's `retry_after`), reuses `chunkForDiscord`/`neutralizeMentions` from `discord-delivery.mjs` rather than a 4th reimplementation; M2 extension: also returns the first chunk's posted `messageId` (from the `?wait=true` response body) so the reply poller has a thread id to find |
...
| `scripts/marjorie/reply-poll.mjs` | M2 (moved from M4): plain-script reply poller — reads founder replies from the brief's Discord thread (`discord-message-id` marker comment locates it) and relays each as an issue comment, idempotent via `<!-- relay-id: ... -->`; never an agent step; M5: its Discord GET/backoff and root filter now live in `lib/discord-bot.mjs`, and it runs as `bot-chat-poll.yml`'s second step |
| `.github/workflows/marjorie-reply-poll.yml` | M2: `workflow_dispatch` job invoking `reply-poll.mjs` under `environment: social`; dispatch-only since M5 (its :03/18/33/48 schedule moved into `bot-chat-poll.yml`) |
...
| `scripts/marjorie/chat-post.mjs` | M5: `save` (agent writes `.scratch/out/chat-reply.md` + summary without a Write tool), `thread` (starts the reply thread; `skip=true` on a duplicate run), `post` (a saved reply only, via the bot's webhook, first attempt only; mentions neutralized and `ref:` lines defused before the 1800-char cap so it is always one message; no reply → sends nothing, `result=missing`), `finish` (`chat-delivery.mjs` read first — a failed read sends nothing; settled → nothing, replied → ✅, notified → ❌, else one referenced bot-token `[chat failed]` notice then ❌; metadata-only `💬 chat:` turn log with `<!-- chat-id -->`, written only by the run that reacted — public repo); tests: `chat-post.test.ts` |
...
| `scripts/doorbell/lib/clock-core.mjs` | M7 clock, pure: POSIX cron matching in UTC, `dueRows`, `clockTick` (skip a slot any run already covers since slot − 60 s; retry a failed dispatch each minute; give up 10 min past the slot), `nextFires`, `parseSchedule`, `clockLiveFrom`/`readRemote`; tests: `clock-core.test.ts` |
| `scripts/doorbell/lib/clock.mjs` | M7 clock I/O: a tick 5 s past each minute, and a 10-minute re-read of `schedule.json` and the `CLOCK_LIVE` line from main's raw files (no key), keeping the pinned copy when that fails; tests: `clock.test.ts` |
| `scripts/doorbell/schedule.json` | M7 clock table: one row per cron slot in `.github/workflows/`, with the inputs that make a dispatch behave like that schedule firing (`watchdog.yml` `schedule`, `community-mailer.yml` `mode`, `backup-restore-drill.yml` `page`, `dry_run: false` where it defaults true); `schedule.test.ts` fails CI until the table and the workflows agree |
| `docs/ops/doorbell.md` | M7: operating the doorbell on the Hermes VM host — files, the journal lines of a healthy start, stop/start, updating the pinned tag, flags, keys (the GitHub key expires 2027-09-13), troubleshooting |
...
| `docs/specs/marjorie-overhaul/m5-chat.md` | M5 spec: a 5-min Discord poll claims founder messages with a 👀 reaction, dispatches a per-message Opus chat routine (Marjorie acts then answers, Tree read-mostly), a `run:` step posts the reply in-thread and reacts ✅/❌; bot token never in the agent env |
