...
| `scripts/marjorie/lib/clock-watch.mjs` | M7: the poll's clock watch, only while `CLOCK_LIVE` — the newest run named `bot-chat-poll · clock` on main since `CLOCK_LIVE_SINCE` (30 min grace; at most 24 h back); none, or 20 min or older, with no open `Clock is not firing` alert → `bot-chat-alarm.yml` `clock-silent`; never fails the poll; also used by `chat-alarm.mjs check`; tests: `clock-watch.test.ts` |
...
| `.github/workflows/bot-chat-poll.yml` | M5: the one Discord poll job (`environment: social`, `actions: write`, `issues: write`) — `chat-poll.mjs poll`, then `reply-poll.mjs` — every 5 minutes (this public repo's Actions minutes are not billed — only private repos draw on the org's $10/month hard stop); `BOT_CHAT_ENABLED=false` kill switch, `dry_run` input; M7: a `clock` input set by the home-server clock names the run `bot-chat-poll · clock`; watched by `watchdog.yml` |
...
| `scripts/marjorie/chat-alarm.mjs` | M7: `check` for `bot-chat-alarm.yml` — `stuck` re-reads Discord (`readDeliveryState`) and the runs, and ends with no alert when a reply, ✅, ❌ or notice is there; titles `Chat reply stuck · <Bot> · <id>` or the standing `Doorbell is not answering` / `Doorbell dispatch is failing` / `Clock is not firing` (`clock-silent`, which ends with no alert once the clock has started the poll within 20 min); `alert` opens it and starts both routines independently, each time-bounded; a body of ids, links, run state and age, never message text; tests: `chat-alarm.test.ts` |
...
| `scripts/doorbell/longlive-doorbell.service` | M7: systemd unit — own user, `Restart=always`, `RestartSec=30`, `NoNewPrivileges`, `ProtectSystem=strict`, `ProtectHome`, `PrivateTmp`, `StateDirectory=longlive-doorbell` (the clock's handled slots) |
| `scripts/doorbell/lib/clock-core.mjs` | M7 clock, pure: POSIX cron matching in UTC, `dueRows`, `clockTick` (due rows side by side; skip a slot any run already covers since slot − 60 s; time and `CLOCK_LIVE` rechecked before each dispatch; retry each minute, give up 10 min past the slot), `policyProblems` (what a table from main may do without a new tag), `parseCommit`, `nextFires`, `parseSchedule`, `readRemote`; tests: `clock-core.test.ts` |
| `scripts/doorbell/lib/clock.mjs` | M7 clock I/O: a tick 5 s past each minute; handled slots saved to `STATE_DIRECTORY`; every 10 minutes, one serialized read of one commit of main (no key) — table and `CLOCK_LIVE` line at that sha, never an older commit, only inside the pinned policy — keeping the pinned copy otherwise; tests: `clock.test.ts` |
