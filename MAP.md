...
| `scripts/marjorie/chat-workflows.test.ts` | M5: text assertions over every deployed chat routine — `run-name` matches `runTitle`, dispatch-only with a per-message group, no Discord secret in the agent job, secret-holding jobs are env-scoped and check out main, artifacts deleted |
