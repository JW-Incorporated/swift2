...
| `scripts/marjorie/lib/submissions.mjs` | C2: 24h issue counts by label (`user-feedback`, `feedback`, `intake`, `link-submission`) for the brief's **Since yesterday** section — client-side date filtering, never `gh issue list --search` (forbidden for repo-scoped cloud sessions, `scripts/lib/gh.mjs`) |
