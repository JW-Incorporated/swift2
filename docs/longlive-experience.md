...
apps/web/
  ...
  components/longlive/
    ...
    NoEmbedFallback.tsx            what a video record renders INSTEAD of the embed when it has
                                   no official upload: "Not available to watch here" + a link to
                                   its first citation. Never let a card be inert (#2050)
  ...
  lib/longlive/
    ...
    video-affordance.ts            PURE rules for "what can this card DO with its footage":
                                   watchAffordance() (embed / link out / nothing — total over
                                   VideoNote, so no surface can render an inert card, #2050),
...
| Era videos | `EraVideos` | inline rail in `EraSection` ("Videos & appearances"); official-upload embeds (via `MomentVideo`). Two families: works she made, and appearances (interview / award speech / speech / press event). A record with **no** `youtubeId` is NOT a bare metadata card — it renders `NoEmbedFallback` ("Not available to watch here" + a link to its first citation). 19 records are in that state, 8 of them permanently (theatrical/streaming-exclusive, so #721's content pass cannot fix them). A test over `videos.generated.ts` asserts every record resolves to an embed or a link, so an inert card fails CI (#2050) |
