...
|---|---|---|
| Era scroll | `EraStream` → `EraSection` | hero + lyric + `EraMedia` + moment grid + pivot strip. A moment carrying `video` shows a **play badge** beside its card (a sibling of the card button, never nested inside it): tapping the badge plays inline in the feed, tapping the card still opens `MomentDetail` (#2051) |
...
| Videos filter | `EraSection` chip row | second filter axis beside the category chips: shows everything watchable in the era (every video record + moments carrying footage), de-duped by `youtubeId`. Mutually exclusive with the tag chips — selecting one clears the other. Rules in `lib/longlive/era-feed.ts` |
