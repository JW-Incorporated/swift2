...
|---|---|---|
| Era scroll | `EraStream` → `EraSection` | hero + lyric + `EraMedia` + moment grid + pivot strip. A moment carrying `video` shows a **play poster** (the video's YouTube thumbnail + play glyph, `PlayPoster`) inside its card: tapping the poster plays inline in the feed, tapping the card still opens `MomentDetail` (#2051). The poster is a DOM sibling of the card button — never nested, which would nest interactive elements — but sits INSIDE the card's border, because the box is drawn by a wrapper rather than by the button (`lib/longlive/card-chrome.ts`). #2055 shipped it as a pill outside the box and it read as "no video here" (#2057). When two moments in one era embed the same `youtubeId`, only the first in feed order plays it; the later one keeps its full text and its own detail-page embed (`inlineVideoMomentIds`) |
