...
| VOICE | 🟡 | nobody | **Regressed after being closed.** 16 open `content.voice` findings in the shipped corpus (13 surname-overuse, 3 wire-attribution), first appearing 2026-08-09. The gate closed partly on the promise that the scan enforces this nightly; the scan is weekly | — |
...
| ERRORS | 🟡 | agent | `unverified` — P0 is 0 and the filable P1 queue is 4 tickets, but the gate says "full criteria": the agent factual + image passes have never run, the nightly scan that is meant to keep this queue current runs weekly, and its ticket filing has produced nothing since 2026-08-02. The queue can neither be drained nor trusted | #1869, #552 |
...
**Count: 🟢 5 · 🟡 6 · 🔴 1.** Of the 7 non-green: **3 blocked on a founder**
(SCAN, LEGAL, BACKUPS), **1 on an agent** (ERRORS), **3 on nobody** (DEPTH,
VOICE, WORTHY). Only the three founder rows are genuinely scarce; the other
four are unclaimed work.
...
| VOICE | nightly | Karen — content-safety scan (the regression signal this gate closed on) | `0 9 * * 0` ⚠️ weekly, below the claim since 2026-07-25 | `docs/agents/runners.md` |
...
**Three gates rest on a nightly scan that has been weekly for 17 days.** That
is the single most important line in this file. It is not a bug in the
throttle — the cost decision was deliberate and approved — it is that nothing
propagated it to the criteria that depended on it. Rule 4 and the checker
...
|---|---|---|---|
| DEPTH | **Content depth: J3.5** — Midnights + TTPD at Active-tier, relationship/sighting/fashion-weighted (fixed launch gate, decisions 2026-07-04) | 🟡 moving, **partly unverified** — 2026-08-11 re-score: the July authoring waves all landed (Era Secrets batches 1–4, the 1989→reputation rupture arc #706/#711, folklore/evermore #734, debut origin story #718, round-3 defining events #865). Coverage today (`npm run content:coverage`, 733 items): midnights **94**, showgirl 87, 1989 64, evermore 64, tortured-poets **61**, speak-now 60, red 58, lover 56, debut 52, fearless 49, reputation 47, folklore 41. Midnights is now the deepest era. **But the gate's own stated measure — rows per month against `docs/content-ops/depth-rubric.md` — has never been computed.** No audit artifact exists after 2026-07-09 and no issue owns it, so "at Active tier" is unevidenced in both directions. TTPD `relationship` = 2 items is the weakest weighted axis in the corpus and is exactly the axis J3.5 named. `content.depth-deficit` = 26 (rollup #1719, open + unassigned); 69 items flagged for a depth pass. Joey's untimed spot-check: no evidence it ever happened. Content Shift runs daily as a Vault Run lane but is a ≤2-items/day news lane — it is not burning a depth gap list, because no gap list exists | Run the rows-per-month × rubric audit → gap list → queue it; then Joey's spot-check. **Blocked on nobody** — unclaimed, needs no decision. (Sub-item #1241, depth-fleet trigger enablement, *is* founder-blocked but is not what holds this gate) · Content desk |
| VOICE | **Voice: the Swiftie bar** (#461) — Taylor-not-Swift rule + fan-editor voice, enforced not aspirational | 🟡 **REGRESSED — was 🟢, reopened 2026-08-11.** The build half is real and stands: the checker shipped (`scripts/content-engine/checkers/voice.mjs` — surname-overuse, ai-tell phrases, wire-attribution), and the retro passes fixed 55 items on PR #1633 (07-30) and 32 more on #1813 (08-06). The gate was flipped 🟢 on PR #1811 (08-06) on a fresh 0-findings scan. **It is no longer 0.** Running the shipped checker against the corpus at `68d71fa` on 2026-08-11 returns **16 findings** — 13 `content.voice.surname-overuse` (1989, debut, evermore, fearless, midnights, red, showgirl ×2, tortured-poets ×3, speak-now theories ×2) and 3 `content.voice.wire-attribution` (evermore, showgirl, tortured-poets). Karen's own committed reports show it starting: `docs/audits/engine/` has 0 voice findings every day 07-30 → 08-08, then 4 on 08-09, 4 on 08-10, 6 on 08-11. **Why nobody saw it:** the gate closed on the sentence "Karen's nightly scan now enforces this permanently — any future item authored in wire-voice gets caught the next night automatically." That sentence was already false when written — the scan went weekly on 2026-07-25, 12 days before the flip. The fallback ("any regression reopens via a new `cie` ticket") could not fire either: `cie` filing has produced nothing since 2026-08-02 (#1869). Two independent safety nets, both silently disabled, on the one gate that was declared permanently self-enforcing | Fix the 16 findings; re-run the checker; only then re-close. **Blocked on nobody** — the checker names every item, no decision needed. Do not re-close 🟢 while the cadence claim above carries ⚠️ · Integrity + Content desks |
...
- **SCAN's criterion vs Karen's cost.** Relax to weekly, restore nightly, or
  ...
  VOICE and ERRORS cannot be trusted to self-report.
...
- **`docs/launch-readiness.md` is edited by three open PRs at once** (#1889
  LEGAL, #1890 BACKUPS, and this re-score). The rows here already incorporate
  both of theirs; on conflict, take this file's version of the LEGAL and
  BACKUPS rows, which is theirs plus the blocked-on column.
