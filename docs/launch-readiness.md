...
| G-E | **Karen nightly, actually nightly** — merged today (#139) but never yet run on schedule; 2026-07-11 full scan pending in this session | 🟡 two consecutive scheduled runs (reports #612/#626, both merged 07-14); **engine fix #625 merged 2026-07-14** (CDN refusals → unverified, never P1) — tonight's run is the first with the image layer expected live, so the 3-clean-pass count can finally start | Watch tonight's run; land the remaining #613 fixes (preflight canary, REST port); count 3 clean passes — image-blind runs don't count as clean · Integrity desk |
