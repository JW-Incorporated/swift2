...
| BACKUPS | 🟡 | founder | **2026-09-05: engineering side assigned to sffan15-sys on #680 (Fable-triaged, t_b3581eb7); the founder-only click is unaffected.** Joey must read the Supabase plan + automated-backup/PITR status off the dashboard, then run one drill against production's own bytes. The drill itself is built and passing. **2026-09-04: the production-bytes drill no longer needs a local checkout or hand-pasted credential — `.github/workflows/production-backup-drill.yml` (`Run workflow`) reuses the existing `SUPABASE_DB_URL` Actions secret and restores read-only into a throwaway Postgres inside the job.** What remains is genuinely founder-only: reading the plan/backup-status off the Supabase dashboard (no programmatic access exists), and clicking "Run workflow" once (tracked as HUMAN-ACTIONS.md #23) | #680 |
