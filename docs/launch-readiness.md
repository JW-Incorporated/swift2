...
| BACKUPS | 🟢 | nobody | Nothing — production-bytes drill **PASSED end-to-end** 2026-09-06 ([run 34057210905](https://github.com/JW-Incorporated/swift2/actions/runs/34057210905)): 35 tables · 8298 rows · 11.27 MB backed up from production and restored into a scratch Postgres with every table's row count and content checksum matching, after PR #3931 fixed the restore loader to skip `generated always ... stored` columns (`egg_ledger.lag_days`). Plan/backup-status was already recorded 2026-08-30 (Joey: Supabase Free, no platform backup/PITR); the resulting risk was ruled **not a founder decision** and mitigated at zero spend by the scheduled Layer-B backup workflow (FR-t_a0ad2392-4, 2026-09-06) rather than a plan upgrade. Nothing founder-only remains | #680 |
...
**Count: 🟢 8 · 🟡 4 · 🔴 0.** Of the 4 non-green: **1 on an agent** (ERRORS),
**3 on nobody** (DEPTH, WORTHY, SCAN). BACKUPS flipped 🟢 2026-09-06 (see
row above); the remaining four are agent work
...
| BACKUPS | **Backups (G4)** — Supabase restore runbook tested once | 🟢 **done — flipped 2026-09-06.** Production-bytes drill ([run 34057210905](https://github.com/JW-Incorporated/swift2/actions/runs/34057210905)) passed end-to-end: 35 tables · 8298 rows · 11.27 MB backed up from real production and restored into a scratch Postgres, every table's row count and content checksum matching (including `egg_ledger`, 37 rows). Two restore bugs had to be fixed first, both closed: #3926 (Supabase `auth`-schema shim for non-Supabase restore targets + pipefail so a swallowed exit code can't fake green) and #3931 (`loadTable()` now builds an explicit, `pg_attribute`-derived column list per table, excluding any `generated always ... stored` column, instead of a bare `select r.*` that tried to write into `egg_ledger.lag_days`). Plan/backup-status was recorded 2026-08-30 (Joey: Supabase Free, no platform backup/PITR); FR-t_a0ad2392-4 (2026-09-06) ruled the resulting risk mitigated at zero spend by the daily Layer-B scheduled backup rather than requiring a plan upgrade, so no further founder click is needed | None gate-blocking. A drill regression reopens this automatically via the next scheduled/manual run · Build desk |
