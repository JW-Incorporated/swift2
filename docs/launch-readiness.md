...
| ALARMS | **Watch desk minimum (L1)** — uptime/error paging on the authoritative user path + analytics baseline | 🔴 not built; analytics DECIDED 2026-07-11 (#477 → A, Vercel Web Analytics — Joey); scoped and filed 2026-07-17 (#799) | Wire Vercel Web Analytics + extend the existing daily smoke check to hourly/multi-route (#799) · Watch/Build desk |
| LEGAL | **Legal pages (G5)** — privacy + ToS covering feedback data (already collected!), minors, analytics consent | 🔴 nothing exists; scoped and filed 2026-07-17 (#800) | Draft privacy + ToS for IP-counsel review (#800) · founders + Content desk |
