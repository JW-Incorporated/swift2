...
| BACKUPS | **Backups (G4)** — Supabase restore runbook tested once | 🟡 moving — 2026-08-11: **the restore is built, executed and passing** (`scripts/backup-restore-test.mjs` + runbook `docs/backup-restore.md`, decisions 2026-08-11). Real run, real evidence: 14 tables / 1901 rows / 3.24 MB backed up and restored into a scratch database in 15.2 s — schema fingerprint match, all 14 per-table content checksums match, 8/8 named spot checks byte-identical (jsonb themes, a 6 KB track dossier, the `moment`→`month_item` FK, and all four runtime-only `news_*` tables). Negative control run too: deliberately dropping a row and altering a title made it exit 1 and name both tables, so the verification is not a rubber stamp. Monthly CI drill wired (`backup-restore-drill.yml`, ~3 Actions-min/month, also triggers on any `supabase/migrations/**` change) so it can't silently rot. Scoping finding that shrinks this gate: schema and content are both **in git** (`supabase/migrations/**`, `supabase/seed/**`) and the live site renders from the committed generated vault, not Supabase — the only state that exists nowhere else is the four `news_*` tables, `news_source.last_polled_at`, and generated uuids. **Two things still red-block 🟢, both Wyatt's:** (1) nobody has confirmed the Supabase plan / whether automated backups + PITR are on for this project (needs the dashboard — on the free plan there are none), (2) no drill has yet run against **production's own bytes** — the executed one used a source built from repo seeds | Wyatt: read the plan + backup/PITR status off the Supabase dashboard into `docs/backup-restore.md` §6; then one real-data drill (`--source "$SUPABASE_DB_URL" --target <scratch>`, read-only source) logged in §6 · Build desk |
