...
The `news-worker` is the only workflow that may read the worker database, because its existing secret-backed execution already performs the Google News RSS cycle. Extend that workflow to emit a separate credential-free JSON receipt at `docs/content-ops/social-source-status.json` on the existing `news-digest` branch. It is a bounded cross-workflow interface, not a new database or source row. Its strict schema contains only `schema_version`, `generated_at`, `lookback_start`, and one status record per configured social source: deterministic source name, `last_polled_at`, `status` (`lead`, `no_lead`, or `unavailable`), and `lead_count`. It must contain no headline, URL, snippet, publisher, handle beyond the configured source name, database identifier, or secret. The existing publication step becomes two independently attempted file publications: preserve its current optional behavior for `docs/content-ops/news-candidates.md` when that file is absent, but whenever the status receipt exists, publish `docs/content-ops/social-source-status.json` to `news-digest` regardless of the candidate digest’s presence. A failed status-receipt publication fails the workflow after its bounded retry policy; a missing candidate digest must never skip or mask that status-receipt publication.
...
4. Reject duplicates during validation. On every successful sync, idempotently set `is_enabled=false` for any existing `social-watch:<platform>:<handle>` row whose account is disabled or absent from the validated watchlist; enabled accounts are the only rows kept or set to `is_enabled=true`. Never delete existing rows in this task. The sync runs as a dedicated step in `.github/workflows/news-worker.yml` on every scheduled and manually dispatched run, using only that workflow’s existing `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` secrets. Step order is `npm run validate:social-sources`, then `node scripts/social-source-sync.mjs`, then the existing `npm run news --workspace @swift2/worker` ingestion cycle, then digest/receipt emission. Validation or sync failure fails the job before ingestion, so a stale enabled row cannot collect after its approval is removed. The migration seeds the initial pattern only; this workflow step is the sole recurring reconciliation path.
...
7. Extend the existing secret-backed digest emitter to write the strict, non-secret `docs/content-ops/social-source-status.json` receipt defined above. Split the existing `news-digest` publication step into independently attempted candidate-digest and status-receipt publications: retain the candidate digest’s current no-file no-op, but always attempt and require successful status-receipt publication whenever that receipt is produced, even if `docs/content-ops/news-candidates.md` is absent. Add a publication fixture/test for `news-candidates.md` absent plus `social-source-status.json` present; it must prove that the status receipt is published and that a publication failure makes the job fail. Also test `lead`, `no_lead`, and `unavailable` status derivation without retaining raw items; include an adapter failure and raw-item upsert failure fixture that both yield `unavailable`, an unrelated `news_source` fixture that proves it cannot enter the receipt, and disabled/removed watchlist fixtures that prove their rows are inactive and omitted. The receipt is the only permitted creator-lead input to Task 4.
