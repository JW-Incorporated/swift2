...
> **Superseded 2026-09-12 — architect-directed redesign, `docs/decisions.md` 2026-09-12 (read that entry; the paragraphs below are kept as the record of what was specified).** The honouring rule below keyed on message identity (`approval.message` naming "that exact message") and was the root cause of three failed adversarial-review rounds on PR #4139: `social-approval-notify.yml` re-posts the header and every unstamped brief on every `synchronize` (including the poll's own pushes) and the digest re-posts daily, so several messages per target is the normal state, and any rule anchored on one message id has an unbounded class of ways to be looking at the wrong one. As built: the poll **listens on the union of every window message** naming a PR, at any SHA (a ❌ anywhere wins, the latest reply anywhere wins), and gates minting and merging on **v3 stamps that sign the head SHA** — `cleanSince(approval.sha, head)` (every changed path is a `social/queue/**.json` that is deleted at head or validly stamped at head) plus `selfClean(F)` (the file's own bytes differ only in `approval`/`body`/`edit`), per file, header-independent; a reaction on a message at SHA *S* may mint *F* only if *S* is head, or `cleanSince(S, head)` holds and *F* is untouched in that range (✅ and ✏️ alike). Drift is recovered by a fresh ✅ on the newest brief; one notice per PR per 24h names the offending paths. The signature payload **is** versioned (v2 stays valid), contrary to "none should be made" below. Ledger rows are derived from state every run (approve/edit from each valid stamp, reject from a ❌+reason whose file is gone) and deduped, and the `edit` object gains `reply`. `social-poster.yml` **is** a second writer to `social-ledger` (contrary to §1b's "nothing races"); it owns `social/queue|posted|failed` there and rebuilds its tree from the fresh tip on every retry so the poll's `social/feedback` rows survive.
...
- **`edit` stays out of the content hash**, and the signature payload is left untouched. *(Superseded 2026-09-12: the payload is versioned — v3 adds the signed head `sha`; v2 signatures stay valid. See the note under "The stale-SHA problem".)*
