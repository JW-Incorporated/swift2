...
4. **The clock** (added 2026-09-14, Joey: "yes, let's run the clock on the
   ...
   the fallback; the clock adds runs, it never removes any.
...
- **New:**
  - `scripts/doorbell/schedule.json`: the schedule table
  - `scripts/doorbell/lib/clock-core.mjs` + `.test.ts`: due rows, the
    skip-if-run-exists rule, retry
