...
- **Kevin moves a ticket to `ready/greenlit` on a human approval comment.**
  M8 adds a narrow equivalent: the exact canonical `marjorie-approval`
...
template, matching URL and marker ids, and typed allowlisted bot author; the
phrase "Plan approved" alone carries no authority. She adds no label and moves
...
routines' next scheduled times (Kevin's daily desk 15:13 UTC, Austin 21:00 UTC).

An ambiguous chat yes (two candidates) gets the M5 rule: act on nothing, ask
which, and list the candidates. An ambiguous reaction adds link-only metadata
to the brief so its next render can report the ambiguity; the poller gains no
Discord-post authority. All ordinary reply relays are also link-only.
