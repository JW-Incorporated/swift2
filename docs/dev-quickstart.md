...
- **Never fabricate content.** (Real + sourced — still stands.) **Media policy** (`docs/decisions.md` 2026-07-08, **rehost ban reinstated then re-reversed 2026-07-09 — read that entry, it's current**): original summaries in our own words + links (never paste bodies/lyrics — see the 2026-07-09 entry for the lyrics exception); oEmbed still preferred for social posts, but **rehosting real internet photos (paparazzi/press/agency) is allowed** — a knowing risk acceptance. Reference/comparable images must still be visibly labeled as such. Monetization needs IP-counsel review. UNOFFICIAL — no affiliation copy.
