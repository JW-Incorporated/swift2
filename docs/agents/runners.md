...
**Requirement (Joey, 2026-07-11): the majority of token spend runs on
Wyatt's account.** Joey's account only *finds and routes* work (cheap
curation — Marjorie); Wyatt's account *solves* work (authoring, building,
walking, checking). Standing operational assumption (Joey, same day): we
have effective command-line access to Wyatt's machine via Joey→Wyatt chat —
any prompt/command Joey relays gets run there, so Wyatt-side setup is a
paste away, never a blocker. Every scheduled runner is registered here with
...
|---|---|---|---|---|---|
| Marjorie — 6 AM brief | `0 13 * * *` | Fable | [`runner-prompts/marjorie-brief.md`](runner-prompts/marjorie-brief.md) | **Joey** | Finds/routes work — light curation |
| Marjorie — 8 PM delta | `0 3 * * *` | Fable | [`runner-prompts/marjorie-delta.md`](runner-prompts/marjorie-delta.md) | **Joey** | Same |
...
1. Wyatt (or his Claude Code session) creates the four Wyatt-side routines
   from this registry: same name, cron, model, and the prompt file's exact
   contents, via `/schedule` or the RemoteTrigger API. (Marjorie's two stay
   on Joey's account by design.)
2. Wyatt comments "live" on the handoff ticket with his routine IDs.
3. Joey's copies of Austin ×2, Nils, and Content Shift get **disabled**
   (kept, disabled, as warm spares — the kill-switch doc covers both sets).
