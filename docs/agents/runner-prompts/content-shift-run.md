...
2. Deterministic queue check per the charter's priority order: (1) open intake-labeled issues with sources attached/findable, (2) open experience-labeled tickets by severity, (3) launch-gate content work per docs/launch-readiness.md G-A. WIP check: >=3 open content-shift PRs → exit. Empty queue → exit fast.
