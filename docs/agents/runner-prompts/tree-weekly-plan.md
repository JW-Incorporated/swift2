...
0.7. **Answer Marjorie's asks of you — normal Monday runs only; a `mode=replan` run skips this** (L1,
   ...
   - **It's marked as contradicting another ask** (its body says
     ...
     waiting on a founder; once one comments on either issue, do what they
     said.
   - **It's inside your hard limits** — almost always a change to
     `social/calendar.md`: note the number, make the change when you rewrite
     the calendar in step 6, and close it only **after** step 9's PR is open
     — `gh issue close <n> --comment "Done in #<plan PR>: <one sentence>"`.
     A run that dies before the PR opens leaves the ask open, which is
     correct.
   ...
   - **It contradicts one of your own asks of Marjorie** — one still open
     (`gh issue list --label tree-filed --label desk:ops --state open`) or
     one you're about to make in step 9: don't act on it; say so in a
     comment and, for a new ask, set `contradicts`. You never settle a
     contradiction — a founder does.
...
3. **Audit the last 7 days.** Run `node --use-env-proxy scripts/social/weekly-scorecard.mjs` (read-only, added Stage 2 2026-08-23) for the deterministic numbers: posts shipped per platform, follower delta per platform, failed count, distinct opener-pattern count over the last 14 days. Use its output as-is in the report (step 9) — never re-derive or round these by hand. Then, on top of that:
   ...
   - Fast-lane counts (T6) — `closeIntent` records no transition timestamp, so scope by `createdAt` instead (Codex review round 1, LOW 1): an intent's whole lifecycle is only 48-72h (its `deadline`), so `createdAt` within the last 7 days reliably approximates "this week"'s activity without needing a new field. Count `declined`/`expired` intents from `social/inbox/closed/`, and `drafted` intents from **`social/inbox/`** itself (drafted intents are NOT moved to `closed/` — they stay put with a `queueFiles` array; only declines and expiries move). Expiry is the normal outcome for most intents, report it as a single number, not a list of failures; a decline's `declinedReason` is worth a line in the report if it points at a real drafting gap (feed it into the lessons pass, step 3.5, the same as any other rejection reason).
...
3.5. **Distill the lessons ledger** (T5, docs/specs/tree-overhaul/t5-lessons-ledger.md — read it for the full mechanics; this is the summary a run actually executes):
   ...
   - **Before finalizing any new or incremented rule's `So I` text, check it against `docs/marketing/social-strategy.md`.** A literal, faithful transcription of a drafting complaint can still end up banning or narrowing something the strategy document already explicitly approves (e.g., a hook shape, a campaign type, a cadence) — that is a strategy question wearing drafting clothes, not a lesson, and writing it as a binding rule is exactly the unreviewed strategy narrowing the proposal mechanism above exists to prevent. If the rule's plain wording would contradict or narrow an approved strategy element, do not write it as a lesson: route it as one of this week's ≤3 proposals instead (below), quoting the specific conflicting strategy line as evidence, even though the founder's own words were about one post, not the strategy itself. When the wording is genuinely ambiguous between a narrow reading (safe) and a broad one (colliding) — the founder saying "don't do this" about one instance, not "never do this" — prefer the narrower reading; the founder sees every new or incremented rule in this week's brief regardless and can broaden it explicitly next week if that is what they meant.
...
9. **Open ONE PR** — title `Tree: weekly plan — week of <date>`, branch `tree/plan/<date>`, label `tree`. The body **is** the weekly report (`docs/agents/tree.md` § Weekly report format) — this is what mails to the founders verbatim, subject `Tree's weekly plan: <PR title>`, so it has to stand alone as a report, not a routine diff description. Four sections, in this order, each with its own heading:
   ...
   2. **Scorecard** — `weekly-scorecard.mjs`'s numbers verbatim (step 3), plus this week's fast-lane count (T6): how many drafted (`social/inbox/`), declined, and expired (both `social/inbox/closed/`), scoped by `createdAt` per step 3's note above.
