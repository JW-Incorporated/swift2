...
3.5. **Distill the lessons ledger** (T5, docs/specs/tree-overhaul/t5-lessons-ledger.md — read it for the full mechanics; this is the summary a run actually executes):
   ...
   - For every `edit`/`reject` row (never `approve` — those never create or fire a rule): read every active rule first and prefer incrementing an existing one over creating a new one — two rules meaning the same thing are worse than one stated imperfectly. A row maps to at most ONE rule. A row matching nothing creates a new rule, at most 3 new ones this week; name a 4th+ unmatched reason in the brief as "N more things I haven't turned into rules yet" rather than forcing a rule. Incrementing bumps `Times fired`, sets `Last fired`, appends to `Evidence`, and replaces **You said** with the newest quote, verbatim, never paraphrased.
   - Retire a rule when either holds, and only here: **codified** (its `check-drafts.mjs` check landed — `Status: retired`, `Superseded by: check-drafts.mjs:<checkName>`) or **stale** (no firing in 8 consecutive weeks AND ≥10 briefs went out in that window — `Superseded by: —`). A retired rule that fires again reactivates with its history and id intact, never re-created.
   - Any active rule now at `Times fired ≥ 3` with `Codify: —`: file the codification issue (title `codify: L### — <title>`, labels `intake`+`social`, body carrying the rule's **So I** text, every evidence link, a suggested check name/location in `check-drafts.mjs`'s `checkDraft` orchestrator, and acceptance criteria phrased as "a draft that does X fails the checker") and write its number into `Codify`.
