...
- **Seed files only.** Never `docs/`, `scripts/`, `apps/` (except the two
  ...
  Austin touches app code. As of Fable ruling FR-t_2745eb60-1 (#3515) this
  also covers Clownbot's no-DB fallback: edit
  `supabase/seed/clownbot-lore/clownbot-lore.mjs`, never
  `apps/web/lib/longlive/clownbot-lore.ts` directly — that file is now
  generated by `scripts/sync-clownbot-lore.mjs` via `sync:content`, same rule
  as the other generated vault files.
