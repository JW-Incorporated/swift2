...
1. Locate where yesterday's review list actually posted (`docs/kevin.md` § "Decision processing"), checking in order: (a) the most recent OPEN `founders-brief`-labeled issue, for the **most recently posted** comment whose first line is `<!-- kevin-stream2-digest -->` (a same-day re-run appends a new anchored comment instead of editing, per the append-and-supersede convention below — always use the latest one and ignore older anchored comments on the same issue); (b) else the most recent open `kevin-digest` issue ("Kevin Daily Review — <date>"). Re-read whichever you find and parse its checkboxes:
...
3. Check for today's open `founders-brief`-labeled issue titled `Founders' Brief — YYYY-MM-DD` (today's America/Los_Angeles date):
   - **Found (normal mode):** post ONE comment on that issue, first line `<!-- kevin-stream2-digest -->` (edit the existing comment carrying that anchor if one exists; never edit the brief body itself). **No comment-edit tool is available to this session (#3631)** — if an anchored comment already exists from an earlier same-day run, post a NEW comment with the same anchor and a second line `_Supersedes the earlier comment(s) above with this anchor — read this one._`, rather than duplicating without the supersede note. The most recent anchored comment is always the current digest.
...
3. Check for today's open `founders-brief`-labeled issue titled `Founders' Brief — YYYY-MM-DD`:
   - **Found (normal mode):** post ONE comment on that issue, first line `<!-- kevin-stream3-triage -->` (edit the existing comment carrying that anchor if one exists; never edit the brief body itself). **No comment-edit tool is available to this session (#3631)** — if an anchored comment already exists from an earlier same-day run, post a NEW comment with the same anchor and a second line `_Supersedes the earlier comment(s) above with this anchor — read this one._`. The most recent anchored comment is always the current triage.
