...
**Clean handoff between streams, mandatory.** Before starting the next stream, leave the working tree exactly as it was before the current stream touched it: if the current stream committed, its commits live on its own branch/PR and `git checkout -` (or `git switch -`) back to a clean base before the next stream starts its own branch; if the current stream failed mid-edit (uncommitted changes), `git stash push -u -m "kevin-desk: <stream> failure, <date>"` those specific stray files before moving on — **never `git restore`/`git clean -fd`** (repo policy, `CLAUDE.md` § "Never destroy uncommitted work" — those commands are blocked by the guard and would irrecoverably discard a partial fix rather than just isolating it). A stash left behind is a visible, recoverable trace of the failure for a human to inspect later — leave it stashed, don't pop or drop it, and don't let it block the next stream. "Clean" here means: `git status` shows a clean working tree and index (nothing uncommitted, nothing untracked from the failed stream) — a stash entry sitting in the stash list is fine and expected after a failure, and never counts against this. Each stream's own `branch` step must start from that clean working tree/index state. This is what makes the "never mix streams on one PR" rule actually hold in the exact failure case it exists for.
...
1. Locate where yesterday's review list actually posted (`docs/kevin.md` § "Decision processing"), checking in order: (a) the most recent OPEN `founders-brief`-labeled issue, for the **most recently posted** comment whose first line is `<!-- kevin-stream2-digest -->` (edit-in-place is now primary, so normally exactly one; a historical issue from the 2026-09-01–09-06 append-and-supersede window may still carry more — use the latest, ignore older); (b) else the most recent open `kevin-digest` issue ("Kevin Daily Review — <date>"). Re-read whichever you find and parse its checkboxes:
...
3. Check for today's open `founders-brief`-labeled issue titled `Founders' Brief — YYYY-MM-DD` (today's America/Los_Angeles date):
   - **Found (normal mode):** post ONE comment on that issue, first line `<!-- kevin-stream2-digest -->`. **Edit-in-place, restored (2026-09-06, #3631):** `gh`/`GH_TOKEN` are available in this runtime — if an anchored comment already exists from an earlier same-day run, find its id (`gh api repos/{owner}/{repo}/issues/{n}/comments --jq '.[] | select(.body | startswith("<!-- kevin-stream2-digest -->")) | .id'`) and PATCH it (`gh api -X PATCH repos/{owner}/{repo}/issues/comments/<id> -f body=@file`) instead of posting a new comment. Never edit the brief body itself. If `gh` is unavailable, fall back to append-and-supersede: a NEW comment, same anchor, second line `_Supersedes the earlier comment(s) above with this anchor — read this one._` The most recent anchored comment is always current.
...
2. For EACH, read its comments (invariant 7): a later human comment can approve a phased plan, change priority, or say "resolved" — reflect the latest human signal in the bucketing. One narrow M8 relay also counts as approval: an exact three-line `Founder approved this in Discord: <canonical link>` / `Plan approved — ready for the build lane.` / `<!-- marjorie-approval: <same message id> -->` comment whose REST `user.type` is `Bot` and whose login is `claude[bot]`, `claude`, `app/claude`, `github-actions[bot]`, or `github-actions`. Check this with the paginated issue-comments API. The Discord permalink message id must equal the marker id; similar prose and bare marker text carry no authority. A later real founder comment overrides or revokes the relay.
3. Check for today's open `founders-brief`-labeled issue titled `Founders' Brief — YYYY-MM-DD`:
   - **Found (normal mode):** post ONE comment on that issue, first line `<!-- kevin-stream3-triage -->`. **Edit-in-place, restored (2026-09-06, #3631):** same as Stream 2 above — find the existing anchored comment's id (`gh api repos/{owner}/{repo}/issues/{n}/comments --jq '.[] | select(.body | startswith("<!-- kevin-stream3-triage -->")) | .id'`) and PATCH it (`gh api -X PATCH repos/{owner}/{repo}/issues/comments/<id> -f body=@file`) instead of posting a new one. Never edit the brief body itself. If `gh` is unavailable, fall back to append-and-supersede (new anchored comment + `_Supersedes the earlier comment(s) above with this anchor — read this one._`). The most recent anchored comment on the issue is always current.
