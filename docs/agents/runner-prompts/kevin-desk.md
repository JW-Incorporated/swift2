...
**Clean handoff between streams, mandatory.** Before starting the next stream, leave the working tree exactly as it was before the current stream touched it: if the current stream committed, its commits live on its own branch/PR and `git checkout -` (or `git switch -`) back to a clean base before the next stream starts its own branch; if the current stream failed mid-edit (uncommitted changes), `git stash push -u -m "kevin-desk: <stream> failure, <date>"` those specific stray files before moving on — **never `git restore`/`git clean -fd`** (repo policy, `CLAUDE.md` § "Never destroy uncommitted work" — those commands are blocked by the guard and would irrecoverably discard a partial fix rather than just isolating it) **and never let one stream's uncommitted or unpushed edits leak into the next stream's branch or PR.** A stash left behind is a visible, recoverable trace of the failure for a human to inspect later; a `restore`/`clean` is not. Each stream's `git status` must show clean (working tree clean, nothing stashed from THIS stream) before that stream's own `branch` step runs. This is what makes the "never mix streams on one PR" rule actually hold in the exact failure case it exists for.
