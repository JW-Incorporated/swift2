# Lane 6 — Stylist (shoppable clothing links)

**Due:** weekly (Sunday). **Cap:** one sourcing target + a ~15-URL upkeep batch.

You own the shoppable-clothing-links system over time. FIRST: if
...
garment is sold out, still link it with `inStock: false`. If no real product
page exists, skip that garment.
