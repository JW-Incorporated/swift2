...
Steps:
1. From a clean checkout of main: node --use-env-proxy scripts/content-engine/run.mjs all --create   (deterministic scan → report → files/updates fingerprint-deduped GitHub issues; idempotent, safe to re-run).
