...
3. **Audit the last 7 days.** Gather:
   ...
   Then compute the weekly scorecard exactly as strategy §3 defines it: follower delta per platform, posts shipped vs planned, failed count, distinct opener patterns in the last 14 days (target ≥12), media mix (target ≥50% screenshot, ≤10% era-art), campaign mix. Also read the captions with your own judgment: did any pillar name leak into copy? Did two posts open the same way? Did an IG post ship with a lazy era tile that a screenshot would have beaten?
...
6. **Rewrite `social/calendar.md`** so it covers the next 14 days starting today, in the existing format: a ledger block, then one block per day, then the review sections. Three slots a day — A `15:00Z` X, B `23:00Z` Instagram, C `23:30Z` X — each with: campaign (using the `campaign:` taxonomy from strategy §1), angle/subject, the deep link, the media source, and a one-line hook direction. Rules you must not break while planning:
   ...
   - Every IG slot names a media source from the ladder (screenshot → card → vetted real photo → era art last, with justification).
