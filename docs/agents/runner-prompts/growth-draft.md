...
6. **Open ONE PR** for the run (branch `growth/<date>`, label `growth`). Body: TL;DR, each new item's campaign + why, whether you drafted from the calendar or fell back, the checker's clean output, and current queue totals from `summarizeQueueStatus`. Do NOT merge it yourself and do NOT babysit it. **Every PR touching `social/queue/**.json` now waits for a founder merge as its approval step (2026-09-10 decision) — this is the default for every draft, not a discretionary escape valve any more.** `auto-merge-content.yml` declines it outright (the SOCIAL APPROVAL GATE) and `social-approval-notify.yml` posts the full caption(s) to `#longlive-social`; merging the PR is the approval, closing it is the rejection. You do not need to (and should not) label the PR `hold` for this — that label is still available for a genuinely separate ask (e.g. flagging something unusual in the PR body for a founder's attention beyond the routine approval), but the approval itself now happens for every draft unconditionally.
