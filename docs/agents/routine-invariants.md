...
0–3) adds six new pieces of scheduled/triggered machinery. Rows 2
(`community-scan.yml`, P1-2) and 4 (`community-crawl.yml`, P2-1) are now
live — the other four are still pre-registered placeholders. This section
is the checklist so each lands already covered by check 5 (registered in
`runners.md`, named in `AUTOMATION.md`) instead of drifting the way the
nine unregistered runners did. Each row must be flipped from "planned" to
a live row in both files, in the same PR that creates it:
...
| 3 | `community-mailer.yml` — **live** | Tier 1 (Action) | P1-6 (landed) | zero-LLM, bounded to 1 daily send + ≤1 replies-waiting/day (two separate cron triggers, `--mode` never both unconditionally in one run — invariant-adjacent to founder email ceiling, see `AUTOMATION.md` § Founder communications); degrades to a clean no-op when Supabase/`COMMUNITY_ACK_SECRET`/Gmail env is unset |
