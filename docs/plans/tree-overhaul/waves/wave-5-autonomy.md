...
**D. Revocation and retraction.** ❌ on a `posted:<type>` notice within 24h, **no reply required** — everywhere else a ❌ needs a reason first, but here the thing that matters is already public and stopping the next one outranks collecting the reason; ask for it afterwards via the standard nudge. Revoke the grant on `social-ledger` (no queue-file edit — `main` is branch-protected, and none is needed: `approvalStatus` requires an *active* grant, so every policy stamp of that type fails verification at post time and the poster retires those items as unapproved), write a `revoke` row to `social/feedback/` for the next Monday distillation to turn into a T5 lesson, and dispatch `social-retract.yml`: delete on X, delete on Facebook, and for Instagram file a `founder-task` carrying the post link and the exact in-app steps.
