...
| M5 · Talk to the bots (Discord chat) ✓ 09-13 (#4270, #4273, #4275; live proof on #4180; latency follow-up is M7) | **Opus** (touches Tree's charter) | M4 merged, **HA #69 done (four bot permissions on both channels)** | half day | 5-min poll → per-message Opus routine → one short reply in the channel (in the existing thread if the founder wrote there; #4320, 09-14), 👀/✅/❌ reactions as state, Marjorie acts before answering, Tree answers read-mostly; spec `docs/specs/marjorie-overhaul/m5-chat.md`, prompt `waves/m5-chat.md` |
| M6 · Live asks between the bots (event-driven L1 answers) | **Opus** (touches both bots' prompts and L1 filing) | M5 merged; **after the 2026-09-14 Monday L1 cycle** (MR1 checks it unmodified); the relay is M7 | half day | an ask filed by one bot dispatches one bounded answer run of the other within minutes (a `gh workflow run` from the filing step — not `on: issues`, which never fires for `GITHUB_TOKEN` filings); answer runs never file; Marjorie's chat-originated asks filed by a trusted `run:` step with the `loop-ask` marker; one-line echo where the founder wrote (channel or existing thread, #4320); Tree answers now, acts Monday; spec `docs/specs/marjorie-overhaul/m6-live-asks.md`, prompt `waves/m6-live-asks.md` |
| M7 · doorbell ✓ 09-14, clock: waves/m7-clock-v2.md | **Opus** (touches the M5 poll; adds a workflow and a host service) | M5 merged; HA #72–#74 done (Joey) for the live proof; not gated on the Monday L1 cycle, but land it before or after M6's build, not interleaved (both edit the chat routines) | half day | "Long Live Doorbell", a non-AI relay on the Hermes VM host, reacts 👀 in ~1 s and dispatches the chat routine (reply ~2.5–3.5 min, was ~4.5); never posts; the 5-min poll stays as fallback and alarms when the doorbell misses or fails to dispatch; a reply stuck 6 min adds ⚠️ and opens a watchdog alert in `#longlive-marjorie`; spec `docs/specs/marjorie-overhaul/m7-doorbell.md`, prompt `waves/m7-doorbell.md` |
| M8 · Drive to done (ready-shaped build tickets, the founder's yes relayed to the issue, a 48/96-hour chase with a `[DECIDE]` human action; Joey 2026-09-14) | **Codex** (`gpt-5.6-sol`, high) | M6 merged; never alongside another wave (M6 and M8 both edit the chat prompt) | half day | `build-ticket.mjs`, `dispatch-chase.mjs`, ops sweep step 2b, the brief's `stalled 2d+:` line, one charter amendment; spec `docs/specs/marjorie-overhaul/m8-drive-to-done.md`, prompt `waves/m8-drive-to-done.md` |
...
**M5 result (2026-09-13, done):** poll #4270, routines #4273, and fix #4275
(the first live runs failed because claude-code-action refused the poll's
bot-actor dispatch; fixed with `allowed_bots` plus a founder-only check).
- Both bots answered Joey in-thread in about 4.5 min. Each stated its job and
  that the two reach each other through numbered issues. Runs 34786301062 and
  34786302939.
- Power proof: Joey wrote "HA #71 is done"; Marjorie opened #4278 to close it
  and cited it in her reply, about 2m43s after his message. Run 34790569523.
- The failure path was proven live: one `[chat failed]` notice plus ❌, and no
  re-claim.
- Evidence is on #4180.
- **Open:** latency follow-up. An architecture evaluation recommends a non-LLM
  relay on the Hermes VM and event-driven L1 answers; seven founder decisions
  are pending.
