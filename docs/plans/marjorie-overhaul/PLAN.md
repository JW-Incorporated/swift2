...
| M5 · Talk to the bots (Discord chat) ✓ 09-13 (#4270, #4273, #4275; live proof on #4180; latency follow-up pending Joey's decisions) | **Opus** (touches Tree's charter) | M4 merged, **HA #69 done (four bot permissions on both channels)** | half day | 5-min poll → per-message Opus routine → in-thread reply, 👀/✅/❌ reactions as state, Marjorie acts before answering, Tree answers read-mostly; spec `docs/specs/marjorie-overhaul/m5-chat.md`, prompt `waves/m5-chat.md` |
| M6 · Live asks between the bots (event-driven L1 answers) | **Opus** (touches both bots' prompts and L1 filing) | M5 merged; **after the 2026-09-14 Monday L1 cycle** (MR1 checks it unmodified); relay part only if Joey approves decisions 1–5 | half day | an ask filed by one bot dispatches one bounded answer run of the other within minutes (a `gh workflow run` from the filing step — not `on: issues`, which never fires for `GITHUB_TOKEN` filings); answer runs never file; Marjorie's chat-originated asks filed by a trusted `run:` step with the `loop-ask` marker; one-line echo into the founder's thread; Tree answers now, acts Monday; spec `docs/specs/marjorie-overhaul/m6-live-asks.md`, prompt `waves/m6-live-asks.md` |
