...
| M7 · Doorbell: founder messages picked up in seconds ◐ 09-14: doorbell + alarm merged (#4306, #4308, #4309, #4311, #4312), tag `doorbell-v1`, install HA #75; live proof waits on Joey. Clock failed Codex twice → debug ladder → architect ruling; rebuild is `waves/m7-clock-v2.md` (scope question on #4290) | **Opus** (touches the M5 poll; adds a workflow and a host service) | M5 merged; HA #72–#74 done (Joey) for the live proof; not gated on the Monday L1 cycle, but land it before or after M6's build, not interleaved (both edit the chat routines) | half day | "Long Live Doorbell", a non-AI relay on the Hermes VM host, reacts 👀 in ~1 s and dispatches the chat routine (reply ~2.5–3.5 min, was ~4.5); never posts; the 5-min poll stays as fallback and alarms when the doorbell misses or fails to dispatch; a reply stuck 6 min adds ⚠️ and opens a watchdog alert in `#longlive-marjorie`; spec `docs/specs/marjorie-overhaul/m7-doorbell.md`, prompt `waves/m7-doorbell.md` |
