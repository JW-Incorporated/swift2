...
| M7 · Doorbell: founder messages picked up in seconds | **Opus** (touches the M5 poll; adds a workflow and a host service) | M5 merged; HA #72–#74 done (Joey) for the live proof; not gated on the Monday L1 cycle, but land it before or after M6's build, not interleaved (both edit the chat routines) | half day | "Long Live Doorbell", a non-AI relay on the Hermes VM host, reacts 👀 in ~1 s and dispatches the chat routine (reply ~2.5–3.5 min, was ~4.5); never posts; the 5-min poll stays as fallback and alarms when the doorbell misses or fails to dispatch; a reply stuck 6 min adds ⚠️ and opens a watchdog alert in `#longlive-marjorie`; spec `docs/specs/marjorie-overhaul/m7-doorbell.md`, prompt `waves/m7-doorbell.md` |
