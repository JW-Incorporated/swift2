...
| R10 | ~~Generate content-id unions instead of importing data into `types.ts`~~ **SUPERSEDED 2026-09-10** — OS-021 (commit `4cf95edb`, `docs/specs/2026-09-05-one-source-three-surfaces.md`) moved `types.ts` to `packages/experience/src/types.ts` and made `EraId` a hand-written literal union instead of deriving it from generated seed data, eliminating the `types.ts`→content-corpus fan-in this task targeted by a different route. PR #3728 (branch `wt/t_15fbcc21`) closed as obsolete; its target file no longer exists on `main`. Fable-ruled, see kanban t_050e1b21. | 1 | M | R8 |
