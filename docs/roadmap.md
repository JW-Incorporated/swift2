...
| **L4** | **Automated announcements:** era-drop (J7) + feature announcements posted through the same queue-and-ship pipeline as the rest of social. Per-item founder approval was restored 2026-09-10 and is now the founder's own Discord ✅ on the brief in `#longlive-tree`, HMAC-signed (`docs/decisions.md` 2026-09-11, B1) — the notification email is a record after the fact, not the checkpoint. Engagement replies stay human indefinitely | ⬜ post-launch (J7 not yet started; the autopost mechanism itself is already live) |
