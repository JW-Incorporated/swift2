...
Every session starts on Sonnet (`settings.json` → `"model": "sonnet"`). The
orchestrator never grinds; it triages, plans, judges, and reviews, escalating
to Opus (`/model opus`) for the judgment work Sonnet shouldn't spend itself
on. Everything mechanical or exploratory runs in a subagent on a cheaper
model, in disposable context. And above the orchestrator sits Fable — invoked
as the `architect` agent, almost never, for the one call per week that
genuinely needs it.
...
work Sonnet does indistinguishably well. The v3 rule: **the most capable model
...
**Layer 1 — `settings.json`.** Sets the main-loop model to Sonnet for every
session. No remembering, no `/model` — escalation to Opus is a conscious act
typed at the prompt.
...
(Sonnet does work itself that a cheaper agent could have done). If you ever see chronic
...
reroutes (if it doesn't say sonnet, `/model sonnet`). Both percentages read `?`
or drop out before the first message of a session; that's expected.
...
| Work | Who |
|---|---|
| Triage of every request; session orchestration | Sonnet (re-asserted every turn by hook; advisory) |
...
occasionally get moved off sonnet — if the statusline shows the wrong model,
`/model sonnet` puts it back). Add a fourth tracked number while the v3.2 tiering
...
2. Fill in the Project facts block in `CLAUDE.md` by hand. Don't make an agent
   guess what you already know. Do the same for **Who you're talking to** —
   the default there is calibrated to Joey specifically; five minutes fixing
   it for a different owner saves every future session over- or
   under-explaining.
...
- Sonnet triages every message — the hook makes sure of it — and never does
...
- Fable (`architect`) is reserve capacity: rare, logged, escalation-only.
  Invoking it for routine calls is the expensive twin of Sonnet doing grunt work.
