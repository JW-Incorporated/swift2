...
(Fable-approved plan, board `swift2`, Phase 0–3 cards). **`community-crawl.yml`
is the first of these four to land (P2-1)** — each other row below is still a
placeholder the naming P1 card fills in when it ships. Standing rule from the
plan: a human always posts; nothing here auto-posts, auto-comments, or
auto-DMs on Reddit or Facebook.
