...
`.github/workflows/social-approval-notify.yml` posts the full caption(s) to
`#longlive-social` so a founder can decide from the prompt alone. **Merging
the PR is the approval; closing it is the rejection.** A PR that only
...
- `approvedBy` + `approvedAt`: **written by the poster on every successful post** — `scripts/social/lib/git-provenance.mjs` finds the latest commit touching the queue file, then asks GitHub's API (`gh api repos/{owner}/{repo}/commits/{sha}/pulls`, the "list pull requests associated with a commit" endpoint) for that commit's `merged_by`/`merged_at`. This is deliberately NOT derived from local git metadata: two Codex review rounds (2026-09-10, see `DEBUG.md`) found that a GitHub web-UI squash merge's commit author is the original drafter and its committer is GitHub's own bot identity — neither is the founder who clicked "Merge." The API call is the only place that information actually lives. Provenance/audit trail only, never a gate — any failure (no git history, `repo` unknown, `gh` unauthenticated, a network hiccup, or a commit with no associated/merged PR) leaves both fields `null` rather than blocking a post.
