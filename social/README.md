...
- **`failed/`** — anything that failed 3 times in a row, hit an ambiguous transport failure (see "Post-time guards" below — never auto-retried), had an invalid/missing `scheduledAt`, or sat due (past `scheduledAt`) for more than 48h without posting for any reason (repeated failure, a guard skip, a deploy-lag skip, an idempotency skip) — see `failureReason` below. Needs a human look either way.
...
4. **Deploy-lag preflight** — HEAD-checks (falling back to a ranged GET if a host rejects HEAD, and requiring an image content-type) every media URL before an IG or X-with-media publish; unreachable media skips the attempt rather than wasting it on a 404.
