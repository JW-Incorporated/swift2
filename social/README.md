...
- `mediaKind` — **required on every draft that carries media** (the 2026-08-12 **Taylor-photo standard**, Joey's directive after the era-tile grid + issue #2031). Three living values, one dead one:
  - `"photo"` — a **real photograph of Taylor Swift**, and the tile **must live under `/social/library/photos/`** (path-enforced, so a screenshot can't be laundered as a credited photo — and a `site-screen` may NOT point there, so a real photo can't ship uncredited). Requires a mandatory `photoId` from `social/photo-library.json`, plus that entry's exact **`mediaCredit`** and **`mediaSource`**; the validator rejects any missing or drifted binding. Never an AI image, untraceable URL, or rehosted video thumbnail.
