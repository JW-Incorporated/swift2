...
| Cleared-photo corpus | **1 file.** `photos/taylor-lover-eras-minneapolis-2023.jpg` (CC BY 2.0, Michael Hicks, via Wikimedia Commons). To grow it legally under the 2026-08-15 policy, source **CC BY / CC BY-SA / CC0 / public-domain** images from Wikimedia Commons, rehost under `/social/library/photos/`, and record `mediaCredit` + `mediaSource`. `apps/web/lib/longlive/lenses.ts` already carries five license-captured Commons images with credit lines — of those, the All Too Well Eras Tour shot (Paolo Villanueva, CC BY 2.0) is the one with Taylor in frame and is re-usable under its licence. Never a Getty preview comp. |
