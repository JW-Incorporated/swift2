...
| Cleared-photo corpus | **3 files**, all rehosted under `/social/library/photos/` at 1080×1350 (IG-safe, ratio 0.8) per the 2026-08-15 policy — grown 2026-08-24 to close #2218 (was down to 1); `taylor-folklore-eras-inglewood-2023.jpg` pulled 2026-08-25 (#3273 — the sourced Commons file was mislabeled, actually a Speak Now stadium-gown shot, not the folklore set; no genuinely-folklore Wikimedia Commons candidate with Taylor clearly in frame was found as a replacement — see #3273 for the audit trail). Each entry is `filename` — `mediaCredit` — `mediaSource`: <br>• `taylor-lover-eras-minneapolis-2023.jpg` — Michael Hicks (CC BY 2.0), via Wikimedia Commons — `https://commons.wikimedia.org/wiki/File:Eras_Tour_-_Minneapolis,_MN_-_Lover_act_-_4.jpg` <br>• `taylor-red-eras-inglewood-2023.jpg` — Paolo Villanueva (CC BY 2.0), via Wikimedia Commons — `https://commons.wikimedia.org/wiki/File:Eras_Tour_-_Inglewood,_California_-_Red_act_10.jpg` (the same shot already vetted in `apps/web/lib/longlive/lenses.ts`) <br>• `taylor-fearless-eras-inglewood-2023.jpg` — Paolo Villanueva (CC BY 2.0), via Wikimedia Commons — `https://commons.wikimedia.org/wiki/File:Taylor_Swift_The_Eras_Tour_Fearless_Set_Era_(53109821975).jpg` (not yet independently visually re-verified — flag if a future audit finds it wrong too). To grow it further under the 2026-08-15 policy, keep sourcing **CC BY / CC BY-SA / CC0 / public-domain** images from Wikimedia Commons (`commons.wikimedia.org` / `upload.wikimedia.org/wikipedia/commons/...` only — never the `/wikipedia/en/...` fair-use path), rehost under `/social/library/photos/`, and record `mediaCredit` + `mediaSource` on the draft — **and visually confirm the era in the actual downloaded file matches the label before committing; a Commons filename claiming an era is not proof** (the "Folklore Set Era" filename batch on Commons that #3273's file came from was itself checked and found to be systematically mislabeled — 4 of 4 sampled files under that exact filename pattern showed a Speak Now blue caped gown, not folklore). Never a Getty preview comp. |
