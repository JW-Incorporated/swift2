...
4. **Get the reset time, then schedule the resume.**
   > **swift2 override — do NOT schedule.** `CLAUDE.md` § "Never babysit your
   > own PR" forbids arming *any* wake-up in this repo, with no exceptions.
   > Read the reset time and state it, then skip straight to step 5 and stop.
   > The rest of this step is retained only so the fallback below reads in
   > context; it does not apply here.
