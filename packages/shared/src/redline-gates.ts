/**
 * Clownbot — the raw TOPIC GATE data for `redline.ts`.
 *
 * Split out purely to keep `redline.ts` under the repo's 300-line file
 * guideline (this data alone is ~200 lines of regex + rationale comments,
 * ported verbatim and none of it may be deleted — see that file's header).
 * Ported verbatim from clownbot-safety.ts's BODY/HEALTH/SEXUALITY/
 ...
 * redline.ts for the mirror-not-import pointer comment).
 *
 * Extracted 2026-08-23 (PLAN.md Stage 2) from
 * `apps/web/lib/longlive/clown-blocklist-gates.ts`, verbatim — that file is
 * now a thin re-export of this one so the live chat safety path's behavior
 * is provably unchanged.
 */
import type { Gate } from './redline';
...
export const SEXUAL: Gate = {
  input: [
    ...
    // Mirrored from SEXUALIZATION_TERMS (see redline.ts header + the
