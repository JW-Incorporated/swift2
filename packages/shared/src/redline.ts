 ...
 *
 * Extracted 2026-08-23 (PLAN.md Stage 2, proposal docs/proposals/
 * 2026-08-23-knowledge-engine.md §3) from
 * `apps/web/lib/longlive/clown-blocklist.ts`, verbatim — the ingestion
 * worker needs the same screening the live chat route uses. That file is
 * now a thin re-export of this module so the live chat safety path's
 * behavior is provably unchanged; do not fork the two.
 *
 * Ported originally from `clownbot-safety.ts` (build A, V2 rebuild). This
 * file owns the "topic" redlines — categories a message can trip regardless
 * of who is speaking: relationship-existence speculation, health, pregnancy,
 * sexuality, family/minors, legal wrongdoing (accusation), private
 * individuals (location — this is also the "location ladder": broad
 * mentions of a place pass, precise identifiers like an address/zip/tail
 * number/flight path do not), sexual content, politics, and other-artists
 * disparagement.
 *
 * The BEHAVIOUR redlines — impersonation, official/insider/human claims, and
 * manufactured certainty — live in `apps/web/lib/longlive/clown-safety.ts`
 * instead: they are about WHO is speaking / HOW confidently, not about a
 * topic, and `clown-safety.ts` combines both sets into the single
 * `screenInput`/`screenOutput` gate a caller actually wants. `screenTopic()`
 * below is the narrower, topic-only entry point this module owns on its own.
 *
 ...
 * a TS constant). Here the boundary runs the other way too:
 * `packages/shared` cannot import from `scripts/` without breaking the
 * portability contract. `SEXUALIZATION_TERMS` below is a literal, hand-kept
 * copy of `scripts/content-engine/config.mjs` `CONFIG.safety.
 * sexualizationTerms` — if that array changes, update both.
 *
 ...
 * redline-gates.ts to keep this file under the 300-line guideline — see that
 * file's header. */
export {
  BODY,
  HEALTH,
