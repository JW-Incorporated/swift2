/**
 * Long Live — the generic search ranking engine (OS-025, headless core;
 * originally `apps/web/lib/longlive/search.ts`, audit T7 / §E.11).
 *
 * This module owns everything about scoring and grouping that has no
 * dependency on which content types exist or how they're loaded: the doc
 * shape, normalization, tokenization, ranking, and grouping/flattening.
 * Building the actual index — which content modules to read, what a
 * `SearchDoc` looks like for a moment vs. a track — stays app-specific
 * (`apps/web/lib/longlive/search.ts`'s `buildSearchIndex`/`getSearchIndex`)
 * because it reads from `apps/web`-only modules (CONTENT, tracks, theories,
 * videos) that haven't moved into the core yet (OS-022..OS-024). Both
 * renderers can build their own doc list against the exact same ranking
 * engine here.
 *
 * The query/ranking half is pure over an injected doc list so it is unit-
 * testable without touching any real dataset.
 */
 ...
 * extra text — see `makeSearchDoc`), and query terms (from `tokenize`) never
...
/**
 * Doc factory for building an index entry — exported so any renderer's
 * index builder (currently `apps/web/lib/longlive/search.ts`'s
 * `buildSearchIndex`) constructs `SearchDoc`s the same way instead of
 * re-deriving `titleNorm`/`bodyNorm` by hand.
 */
