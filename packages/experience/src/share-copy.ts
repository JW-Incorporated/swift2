/**
 * Long Live — shared share-copy builders + share-target model (OS-038,
 * `docs/specs/2026-09-05-one-source-three-surfaces.md` Phase 3).
 *
 * Moved out of `apps/web/lib/longlive/share.ts` into the headless core (same
 * move `search.ts` made for the ranking engine in OS-025) so `apps/mobile`
 * builds its native share sheet from the exact same copy the web's
 * `ShareSheet.tsx` renders, instead of a second hand-kept-in-sync copy of
 * these strings. Pure and framework-free: no React, no DOM, no `navigator`.
 */
...
/**
 * Every shareable destination in the app (#707, #2105). Moved from
 * `apps/web/lib/longlive/store/search-share.tsx` so both renderers share one
 * definition — the web store re-exports this type unchanged so every
 * existing `ShareTarget` import keeps working.
 */
...
/**
 * The shareable URL for a target, relative to `baseUrl` (no trailing
 * slash). Mirrors `ShareSheet.tsx`'s `shareUrl` builder exactly — moved here
 * (OS-038) so `apps/mobile`'s native share sheet points at the same deep
 * links the web reads on mount (`AppProvider`'s `?item=`/`?lens=`/`?era=`
 * handling), instead of a second, driftable copy of this param mapping.
 */
export function buildShareUrl(target: ShareTarget, baseUrl: string): string {
  const base = baseUrl.replace(/\/$/, '');
  if (target.kind === 'item') return `${base}?item=${encodeURIComponent(target.itemId)}`;
  if (target.kind === 'lens') return `${base}?lens=${encodeURIComponent(target.lensId)}`;
  if (target.kind === 'era') return `${base}?era=${encodeURIComponent(target.eraId)}`;
  if (target.kind === 'track') return `${base}?song=${encodeURIComponent(target.trackKey)}`;
  if (target.kind === 'trackGuide') return `${base}?guide=${encodeURIComponent(target.eraId)}`;
  if (target.kind === 'theoryGuide') return `${base}?theories=${encodeURIComponent(target.eraId)}`;
  ...
  if (target.kind === 'threads') return `${base}?mode=threads`;
  if (target.kind === 'mood') return `${base}?mode=mood`;
  if (target.kind === 'clownbot') return `${base}?mode=clownbot`;
  if (target.kind === 'community') return `${base}?mode=community`;
  if (target.kind === 'merch') return `${base}?mode=merch`;
  // target.kind === 'site' — the bare front door, no params.
  return base;
}
