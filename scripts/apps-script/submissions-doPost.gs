 ...
 *
 * Leading whitespace counts: Sheets ignores a whitespace-prefixed =, but a CSV
 * export opened in Excel does not, so "\t=1+1" would come back to life one hop
 * downstream. Keep this rule identical to neutralizeCell() in
 * apps/web/lib/longlive/submit-link.ts — change one, change both.
 */
