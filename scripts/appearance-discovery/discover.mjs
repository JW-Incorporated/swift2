...
// kanban t_bac31b1a; revised again 2026-09-12, Tree Overhaul T6, to stop
// writing captions): alongside that intake issue, FILE mode writes a
// `social/inbox/*.json` fact sheet (lib/appearance-intent.mjs) — the RSS
// metadata (title/channel/URL) this lane has always restated, and nothing
// else. No caption, no queue write, ever. Tree's own daily draft reads it
// in a later run and decides
// whether (and how) to post; the intake issue stays this lane's own record
// and its `links.issue` points at it. The workflow's own git step commits
// the intent via a PR like any other content PR (no social-draft approval
// gate applies — the intent carries no publishable text).
//
// Usage:
...
// refused (dedupe fail-closed), when any issue create failed, or when an
// inbox intent failed to build — loud beats quiet (see runners.md: a silent
...
import { buildAppearanceIntent } from './lib/appearance-intent.mjs';
  ...
  if (plan.refuse) {
    ...
    for (const c of plan.toFile) {
      ...
      try {
        ...
        issueNumber = Number(url.match(/\/issues\/(\d+)/)?.[1]) || null;
      } catch (e) {
        createFailures.push(`${c.videoId}: ${String(e?.stderr || e?.message || e).slice(0, 300)}`);
        console.error(`  FAILED to file ${c.videoId}: ${e.message}`);
        continue; // no fast-lane intent for a video whose slow-lane lead didn't even file
      }
      ...
      // Fast lane (docs/decisions.md 2026-08-25; Tree Overhaul T6,
      // 2026-09-12, stops writing captions): write a social/inbox/ fact
      // sheet alongside the intake issue instead of a queue draft. Never
      // blocks or undoes the issue that already filed — a bad intent is
      // loud, not fatal, same "loud beats quiet" posture as everything else
      // in this script. The workflow's own git step (appearance-
      // discovery.yml) commits whatever lands here via a PR like any other
      // content PR.
      try {
        const intent = buildAppearanceIntent(c, { now: new Date(now), issueNumber });
        const findings = validateIntent(intent);
        if (findings.length) throw new Error(`invalid intent: ${findings.join('; ')}`);
        const filename = `${intent.id}.json`;
        writeFileSync(join(root, 'social', 'inbox', filename), `${JSON.stringify(intent, null, 2)}\n`, 'utf8');
        staged++;
        console.log(`  staged social/inbox/${filename}`);
      } catch (e) {
        draftFailures.push(`${c.videoId}: ${e.message}`);
        console.error(`  FAILED to stage intent for ${c.videoId}: ${e.message}`);
      }
    }
  } else if (!FILE_MODE) {
    // Dry run: preview what the fast lane would stage, without touching the
    // filesystem or gh — buildAppearanceIntent is pure.
    for (const c of plan.toFile) {
      const intent = buildAppearanceIntent(c, { now: new Date(now) });
      console.log(`  would stage social/inbox/${intent.id}.json`);
    }
  }
