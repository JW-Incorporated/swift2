...
import { hostOf, slugify } from './longlive-sync-shared.mjs';
...
export function isWeakSource(source) {
  ...
  if (WEAK_SOURCE_TYPES.has(source.source_type)) return true;
 ...
 * (docs/content-ops/editorial-voice-and-pipeline.md), so two citations to the
 * same host count once however they are named — the common near-miss is two
 * Billboard articles about the same chart week. Weak sources contribute zero.
 */
export function independentOutlets(sources) {
  const hosts = new Set();
  for (const s of Array.isArray(sources) ? sources : []) {
    if (isWeakSource(s)) continue;
    const host = hostOf(s.url ?? s.source_url ?? s.sourceUrl ?? '');
...
export const SINGLE_OUTLET_LEGACY = new Set([
  ...
  'evermore.mjs#vmas-2022-midnights-handoff',
